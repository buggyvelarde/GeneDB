...
summary() {
    echo "load pfam predictions"
}

loaderHelp() {
    cat <<HELP
Load pfam predictions, from the file produced by the pfam_scan.pl program.
...
Options:
  ...
  -v version
    The version of Pfam that was used to generate the input file
  -k key-type
  ...
  -r
    Reload. If this option is present, existing Pfam predictions for
...
doLoad() {
    ...
    while getopts "do:v:rk:x:$stdopts" option; do
        ...
        v)  programVersion="$OPTARG"
            ;;
        r)  reload=true
        	echo "Reloading domains: existing pfam_scan domains will be deleted!"
            ;;
        k)  keyType="$OPTARG"
        	;;
        x)  case "$OPTARG" in
    ...
    if $reload; then
        java -Xmx256m $database_properties \
            org.genedb.db.loading.auxiliary.ClearDomains "$organism" pfam_scan
    fi
    ...
    java $database_properties \
        org.genedb.db.loading.auxiliary.Load domainloader --program=pfam_scan \
