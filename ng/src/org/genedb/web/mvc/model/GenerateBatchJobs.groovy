...
for (org in orgs) {
    ...
    case "Lucene":
		...
		execLine="java -server -Djava.awt.headless=true -Xmx1524m  -classpath ${cacheClassPath}  org.genedb.web.mvc.model.PopulateLuceneIndices -o ${org} -i ${baseDir}/output/${org} ${dbname} "
        break;

    case "DTO":
        // execLine="ant -f build-apps.xml -Dconfig=${config} -Dorganism=${org} -Ddir=${baseDir}/output/${org} _PopulateCaches "
		// -XX:+HeapDumpOnOutOfMemoryError
		execLine="java -server -Djava.awt.headless=true -Xmx1524m -classpath ${cacheClassPath} org.genedb.web.mvc.model.PopulateCaches -o ${org} --globalFileRoot ${baseDir}/output/${org} "
        new File("${baseDir}/output/${org}").mkdir()
        break;
    ...
    Process p = ["ssh", "pcs4a", "bsub -q ${queueName} -M 3000000 -o ${outFileName} -e ${errFileName} ${scriptName}"].execute()
