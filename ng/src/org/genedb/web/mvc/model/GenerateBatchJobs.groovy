...
for (org in orgs) {
    ...
    case "Lucene":
		...
		execLine="java -server -Djava.awt.headless=true  -Xmx1024m  -classpath ${cacheClassPath}  org.genedb.web.mvc.model.PopulateLuceneIndices -o ${org} -i ${baseDir}/output/${org} ${dbname} "
        break;

    case "DTO":
        // execLine="ant -f build-apps.xml -Dconfig=${config} -Dorganism=${org} -Ddir=${baseDir}/output/${org} _PopulateCaches "
		execLine="java -server -Djava.awt.headless=true  -Xmx1024m -XX:+HeapDumpOnOutOfMemoryError -classpath ${cacheClassPath} org.genedb.web.mvc.model.PopulateCaches -o ${org} --globalFileRoot ${baseDir}/output/${org} "
        new File("${baseDir}/output/${org}").mkdir()
        break;
    ...
    Process p = ["ssh", "pcs4a", "bsub -q ${queueName} -M 2000000  -R select[mem>2000] rusage[mem=2000] -o ${outFileName} -e ${errFileName} ${scriptName}"].execute()
