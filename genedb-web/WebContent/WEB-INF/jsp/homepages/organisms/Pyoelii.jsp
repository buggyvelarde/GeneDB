<div id="homepage-text">
 <a name="The_Organism">
 </a>
 <h2>
  <span class="mw-headline">
   The Organism
  </span>
 </h2>
 <p>
  <i>
   Plasmodium yoelii yoelii
  </i>
  has been isolated from the blood of shiny thicket rats from the Central African Republic, from Brazzaville and from Western Nigeria. Three subspecies are recognized,
  <i>
   P. yoelii yoelii
  </i>
  ,
  <i>
   P. yoelii killicki
  </i>
  and
  <i>
   P. yoelii nigeriensis
  </i>
  . The parasite is readily grown in laboratory mice and rats, where it shows a preference for immature red blood cells or reticulocytes.
 </p>
 <a name="The_genome">
 </a>
 <h2>
  <span class="mw-headline">
   The genome
  </span>
 </h2>
 <p>
  The
  <i>
   P. yoelii
  </i>
  17XNL genome is 23.1 Mb in size, with a karyotype of 14 chromosomes. The G+C content is 22.6%.
Annotation of the
  <i>
   P. yoelii
  </i>
  17XNL genome obtained from genbank. Includes gene models and predicted protein translations.
 </p>
 <a name="Data_Release">
 </a>
 <h2>
  <span class="mw-headline">
   Data Release
  </span>
 </h2>
 <p>
  This genome has been published in Carlton et al., Nature 419 (6906):512-519.     PMID: 15637271
The genome is not curated.
 </p>
 <!-- 
NewPP limit report
Preprocessor node count: 4/1000000
Post-expand include size: 0/2097152 bytes
Template argument size: 0/2097152 bytes
-->
 <!-- Saved in parser cache with key wikidb:pcache:idhash:5729-0!1!0!!en!2 and timestamp 20091130201358 -->
</div>