...
<div id="geneDetails">
  <format:genePageSection className="whiteBox">
    <display:table name="results"  id="row" pagesize="30" requestURI="/BrowseCategory" class="simple" cellspacing="0" cellpadding="4">
        ...
        <a href="<c:url value="/Query"><c:param name="q" value="controlledCuration" /><c:param name="cvTermName" value="${row.name}" /><c:param name="taxons" value="taxonNodeName" /><c:param name="cv" value="${category}"/></c:url>"><c:out value="${row.name}"/></a>
         </display:column>
