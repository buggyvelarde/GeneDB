...
public class BooleanQuestionTag extends SimpleTagSupport {
    ...
    public void doTag() throws JspException, IOException {
        ...
        Map<String, Object> map = (Map<String, Object>) getJspContext().getAttribute(
            WebConstants.MODEL_MAP, PageContext.REQUEST_SCOPE);
    ...
    private void showBoolOrSingleQuery(NumberedQueryI nq, StringWriter buf, int depth)
            throws IOException {
    ...
    private void showBooleanQuery(BooleanQuery q, StringWriter buf, int depth) throws IOException {
        ...
        buf.append("</td>" + prevColouredCell + "&nbsp;</td></tr>\n<tr><td style=\"border-left:"
                + BORDER_STYLE + "; border-right:" + BORDER_STYLE + "\" colspan=\"2\" bgcolor=\"");
        ...
        buf.append("<input type=\"hidden\" name=\"node." + q.getIndex() + "\" value=\"bool."
                + q.getOp() + "." + q.getFirstQuery().getIndex() + "."
                + q.getSecondQuery().getIndex() + "\"></input>");
    ...
    // TODO Change to two sets of buttons : union this query, union this group
    // etc
    ...
    private void showQuestionList(StringWriter buf, int index) {
        buf.append("<select onchange=\"document.forms['" + id + "'].submit()\" name=\"node."
                + index + "\">");
    ...
    private void showIcons(StringWriter buf, int localIndex) {
        PageContext pc = (PageContext) getJspContext();
        HttpServletRequest req = (HttpServletRequest) pc.getRequest();
        String contextPath = req.getContextPath();
        // Remove set from icon name for text versions
        buf.append("<input type=\"IMAGE\" name=\"bop.union." + localIndex + "\" src=\""
                + contextPath + "/includes/images/unionset.png\" alt=\"UNION\" title=\"UNION\"/>");
        buf.append("<input type=\"IMAGE\" name=\"bop.intersect." + localIndex + "\" src=\""
                + contextPath
                + "/includes/images/intersectset.png\" alt=\"INTERSECT\" title=\"INTERSECT\"/>");
        buf.append("<input type=\"IMAGE\" name=\"bop.subtract." + localIndex + "\" src=\""
                + contextPath
                + "/includes/images/subtractset.png\" alt=\"SUBTRACT\" title=\"SUBTRACT\"/>");
    }
