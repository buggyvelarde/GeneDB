...
public class DisplayControlledCuration extends SimpleTagSupport {
    ...
    public void doTag() throws JspException, IOException {
        if (polypeptide != null) {
            ...
            List<Map<Integer,FeatureCvTermProp>> featureProps = new ArrayList<Map<Integer,FeatureCvTermProp>>();
                ...
                if ("CC_genedb_controlledcuration".equals(featCvTerm.getCvTerm().getCv().getName())) {
                    ...
                    for (FeatureCvTermProp featCvTermProp : featCvTerm.getFeatureCvTermProps()) {
                        ...
                        if ("qualifier".equals(featCvTermProp.getCvTerm().getName())) {
                            ...
                            logger.info("count " + totalQualifier + " value "
                                    + featCvTermProp.getValue());
                            totalQualifier++;
                        }
                    ...
                    // List<Feature> temp =
                    // sequenceDao.getFeaturesByCvTermName(featCvTerm.getCvTerm().getName());
            ...
            if (totalRows > 0) {
                ...
                for (int i = 0; i < totalRows; i++) {
                    ...
                    String endTag = "</td>";
                    if (qualifier > 1) {
                        startTag = "<td colspan=\"1\" rowspan=\"" + qualifier
                                + "\" style=\"vertical-align: top;\">";
                    } else {
                        startTag = "<td>";
                    }
                    out.println(startTag + fct.getCvTerm().getName() + endTag);
                    Map<Integer,FeatureCvTermProp> fcp = featureProps.get(i);

                    String pub = fct.getPub().getUniqueName();
                    if (pub.equals("null")) {
                        if (fct.getFeatureCvTermDbXRefs() == null
                                || fct.getFeatureCvTermDbXRefs().size() == 0) {
                            out.println(startTag + endTag);
                        } else {
                            String acc = fct.getFeatureCvTermDbXRefs().iterator().next()
                                    .getDbXRef().getAccession();
                            out.println(startTag + acc + endTag);
                        }
                    } else {
                        String[] sections = pub.split(":");
                        out.println(startTag
                                        + "<a href=\"http://www.ncbi.nlm.nih.gov/entrez/query.fcgi?cmd=Retrieve&db=PubMed&dopt=Abstract&list_uids="
                                        + sections[1] + "\">" + pub + "</a>" + endTag);
                    }
                    if (fcp.containsKey(1)) {
                        out.println("<td>" + fcp.get(1).getValue() + endTag);
                    } else {
                        out.println(startTag + " " + endTag);
                    }
                    // System.out.println(sequenceDao.toString());
                    int size;
                    size = GeneDBWebUtils.featureListSize(fct.getCvTerm().getName(),
                        "CC_genedb_controlledcuration");
                    if (size > 1) {
                        size = size - 1;
                        out.println(startTag + "<a href=\"" + url
                                + "Search/FeatureByCvTermNameAndCvName?name="
                                + fct.getCvTerm().getName() + "&cvName="
                                + fct.getCvTerm().getCv().getName() + "\">" + "(" + size
                                + " others)" + "</a>" + endTag);
                    } else {
                    ...
                    if (qualifier > 1) {
                        ...
                        while (qualifier > count) {
                            ...
                            out.println("<td>" + fcp.get(num).getValue()
                                    + endTag);
                            out.println("</tr>");
                            count++;
                        }
