...
class ReportChanges {
    ...
    void process(def latest, def genes, boolean mungeLocations) {
        ...
        latest.eachRow("select * from feature where type_id=${geneId} and organism_id=${organismId}") {
...
//            System.err.println it.uniquename
            ...
            if (row == null) {
                if (!mrg.uniqueName.contains("RNA")) {
                    //System.err.println("${count++} Problem retriving mRNA for '${mrg.uniqueName}' '${mrg.featureId}'")
                }
                return
            }
            ...
            latest.eachRow("select subject_id from feature_relationship where object_id=${mrnaId}") {
                def row2 = latest.firstRow("select uniquename, type_id from feature where feature_id=${it.subject_id}");
                switch (row2.type_id) {
                case proteinId:
                    if (protein != -1) {
