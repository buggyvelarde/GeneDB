...
public class TempCvDao  {
	...
	public CvTerm findConventionalFeatureForProperty(CvTerm cvTerm) {
		...
		CvTerm soType=null;// = cvService.findCvTermByCvAndName("sequence", featureTypeName);
		if (soType == null){
	...
	public CvTerm findCvTermByCvAndName(String cvName, String termName) {
		try {
			...
			"from CvTerm cvt where cvt.name = :termName and cvt.cv.name = :cvName")
			.setString("cvName", cvName)
			.setString("termName", termName)
			.uniqueResult();
			return type;
		}
		catch (NonUniqueResultException exp) {
			log.error(String.format("Got more than 1 result when should have had one for an cvterm of '%s'/'%s'", cvName, termName), exp);
			return null;
		}
	}
}