...
class PepStatsQuick {
    ...
    public static PepStatValues calculatePepstats(String residuesIn) {
        // if (gene.isPartial()) {
        // gene.setPepStatsWarning("No peptide properties calculated for partial
        // sequences");
        // return;
        // }

        // if (gene.isPseudo()) {
        // gene.setPepStatsWarning("No peptide properties calculated for
        // pseudogenes");
        // return;
        // }
        ...
        if (residuesIn == null || residuesIn.length() == 0) {
            ret
                    .addWarning("No protein statistics calculated - protein sequence unknown");
            ret.setValid(false);
            return ret;
        }

        String residues = replaceResidues(residuesIn, ret);

        try {
            SymbolList aaSymList = new SimpleSymbolList(aaToken, residues);

            ret.setWeight(MassCalc.getMass(aaSymList,
                    SymbolPropertyTable.AVG_MASS, false));

            ret.setCharge(ChargeCalc.getCharge(residues));

            ret.setIsoelectricPoint((float) iep.getPI(aaSymList, true, true));
        } catch (BioException e) {
            // LogUtils.bprintln("WARN: gene '"+gene.getId()+"' had problems
            // when generating pepstats. Maybe stop codons?q");
    ...
    public static String replaceResidues(String peptideString, PepStatValues ret) {
        ...
        if (replacedWarning.length() > 0) {
            ret.addWarning("These protein statistics were calculated by replacing "
                            + "one or more unknown residues ("
                            + replacedWarning.toString()
