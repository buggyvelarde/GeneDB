 ...
 * <ul>
 * <li>PropertyOverrideConfigurer for "beanName.property=value" style
 * overriding (<i>pushing</i> values from a properties file into bean
 * definitions)
 * <li>PropertyPlaceholderConfigurer for replacing "${...}" placeholders (<i>pulling</i>
...
public class PropertyOverrideConfigurerByCoding implements BeanFactoryPostProcessor, Ordered {
    ...
    private Properties retrieveProperties() {
        ...
        try {
            ...
            Method m = ClassUtils.getStaticMethod(holdingClass, "getProperties",
     ...
     * <p>
     * Default implementation will invoke <code>convertPropertyValue</code>
     ...
     * <p>
     * Default implementation simply returns the original value. Can be
    ...
    /**
     * A property resource configurer that overrides bean property values in an
     * application context definition. It <i>pushes</i> values from a
     ...
     * </pre>
     *
     * In contrast to PropertyPlaceholderConfigurer, the original definition can
     * have default values or no values at all for such bean properties. If an
     * overriding properties file does not have an entry for a certain bean
     * property, the default context definition is used.
     *
     * <p>
     * Note that the context definition <i>is not</i> aware of being
     * overridden; so this is not immediately obvious when looking at the XML
     * definition file.
     *
     * <p>
     * In case of multiple PropertyOverrideConfigurers that define different
     * values for the same bean property, the <i>last</i> one will win (due to
     * the overriding mechanism).
     *
     * <p>
     * Property values can be converted after reading them in, through
     * overriding the <code>convertPropertyValue</code> method. For example,
     * encrypted values can be detected and decrypted accordingly before
     * processing them.
     *
     * @author Juergen Hoeller
     * @author Rod Johnson
    ...
    /**
     * Set the separator to expect between bean name and property path. Default
     * is a dot (".").
     */
    ...
    protected void processProperties(ConfigurableListableBeanFactory beanFactory, Properties props)
        ...
        for (String localKey: props.stringPropertyNames()) {
            try {
                processKey(beanFactory, localKey, props.getProperty(localKey));
            } catch (BeansException ex) {
                String msg = "Could not process key '" + localKey
     ...
     */
    protected void applyPropertyValue(ConfigurableListableBeanFactory factory, String beanName,
