...
summary() {
    echo "load iprscan (InterPro) predictions"
}

loaderHelp() {
    cat <<HELP
Load pfam predictions, from the file produced by the iprscan program.
...
Options:
  ...
  -v version
    The version of iprscan that was used to generate the input file
  -k key-type
  ...
  -x notFoundNotFatal
    ...
    that are not found in the database. Useful if iprscan was run against a 
    slightly out of date set of proteins. Default false.
  -r
    Reload. If this option is present, existing InterPro domains for
...
doLoad() {
    ...
    while getopts "do:v:rk:x:$stdopts" option; do
        ...
        v)  programVersion="$OPTARG"
            ;;
        r)  reload=true
        	echo "Reloading domains: existing InterPro domains will be deleted!"
            ;;
        k)  keyType="$OPTARG"
        	;;
        x)  case "$OPTARG" in
    ...
    if $reload; then
        java -Xmx256m $database_properties \
            org.genedb.db.loading.auxiliary.ClearDomains "$organism" iprscan
    fi
    ...
    java $database_properties \
        org.genedb.db.loading.auxiliary.Load domainloader --program=iprscan \
